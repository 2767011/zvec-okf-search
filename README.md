...
| `fts_raw` | zvec BM25 по исходным словоформам | измерения вклада лемматизации |
...
Набор создаваемых индексов задаётся отдельно:

```text
OKF_ZVEC_INDEX_MODELS=e5
OKF_ZVEC_INDEX_MODELS=e5,paraphrase
OKF_ZVEC_INDEX_MODELS=all
```

По умолчанию systemd индексирует только E5. Это сокращает синхронизацию и
занимаемое место примерно вдвое. Веб-интерфейс предлагает только модели, для
которых включён индекс. Изменение списка требует новой синхронизации OKF.
...
в `benchmarks/queries.json`. Команда сравнивает `semantic`, исходный FTS,
лемматизированный FTS и `hybrid`,
...
Конкурентную обработку HTTP-запросов можно проверить без сторонних утилит:

```bash
python scripts/load_test.py \
  --service-url http://127.0.0.1:8765 \
  --token-file ./search-token \
  --mode hybrid --requests 20 --concurrency 4
```

Тест прогревает модель, отключает кэш и выводит throughput, среднюю задержку,
P50, P95, ошибки и число пустых выдач.

На CT 146 при 15 hybrid-запросах без кэша и concurrency 4 перенос embedding
за пределы глобального lock изменил throughput с 2.19 до 10.14 запроса/с, а
P95 — с 2720 до 471 мс. При concurrency 1 средняя задержка снизилась с 630 до
540 мс. Значения зависят от CPU и корпуса, поэтому для другой установки тест
следует повторить локально.
...
Начиная с версии `0.7.0`, код разделён по ответственности:

| Модуль | Ответственность |
| --- | --- |
| `config.py` | константы, переменные окружения и валидация параметров |
| `text.py` | frontmatter, Markdown-чанкинг и русская нормализация |
| `state.py` | блокировки, кэши, метрики и история запросов ИИ |
| `models.py` | загрузка embedding-моделей и инференс |
| `indexing.py` | схема Zvec, построение и атомарная публикация индексов |
| `search.py` | semantic/FTS/hybrid-поиск и ранжирование |
| `web.py` | HTTP API, веб-страницы, авторизация и состояние сервиса |
| `cli.py` | команды `index`, `search`, `benchmark` и `serve` |

Файл `src/okf_zvec.py` оставлен тонкой совместимой точкой входа для прежнего
запуска `python okf_zvec.py`. Устанавливаемая команда вызывает
`okf_zvec_search.cli:main` напрямую.
