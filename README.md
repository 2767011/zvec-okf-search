...
Профиль предзагрузки можно изменить на странице `/status`. Настройка
сохраняется в `OKF_ZVEC_RUNTIME_SETTINGS_FILE`, применяется сразу и имеет
приоритет над переменной окружения. Там же доступны перезагрузка выбранных
моделей и перезапуск сервиса под управлением systemd.
